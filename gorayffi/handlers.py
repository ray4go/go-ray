...
def handle_run_py_local_task(
    ...
    func_name = options.pop(consts.TASK_NAME_OPTION_KEY)
...
def handle_run_python_func_code(data: bytes) -> tuple[bytes, int]:
    ...
    try:
        ...
        return error_string.encode("utf-8"), consts.ErrCode.Failed
...
def handle_new_python_class_instance(
    ...
    return _uint64_le_packer.pack(instance_id), consts.ErrCode.Success
...
def get_handlers(
    ...
    return {
        consts.Go2PyCmd.CMD_EXECUTE_PYTHON_CODE: handle_run_python_func_code,
        consts.Go2PyCmd.CMD_EXECUTE_PY_LOCAL_TASK: functools.partial(
            handle_run_py_local_task, python_func_getter=python_func_getter
        ),
        consts.Go2PyCmd.CMD_NEW_CLASS_INSTANCE: functools.partial(
            handle_new_python_class_instance,
            python_class_getter=python_class_getter,
            class_instances_store=class_instances_store,
        ),
        consts.Go2PyCmd.CMD_LOCAL_METHOD_CALL: functools.partial(
            handle_class_instance_method_call,
            class_instances_store=class_instances_store,
        ),
        consts.Go2PyCmd.CMD_CLOSE_CLASS_INSTANCE: functools.partial(
