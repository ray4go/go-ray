...
import gorayffi
from gorayffi import consts, cmds, funccall
...
def load_go_lib() -> cmds.GoCommander:
    if not state.golibpath:
        raise Exception("init() must be called first")

    cmder = gorayffi.load_go_lib(state.golibpath, handlers.handlers)
...
def make_sub_class(cls, name: str, namespace: str = "", **members):
    if namespace:
        namespace = namespace + "."
